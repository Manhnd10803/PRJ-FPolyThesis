            ...
            if (!$reply_to) {
                ...
                switch ($type) {
                    case 'post':
                        ...
                        $message = Auth::user()->first_name . ' ' . Auth::user()->last_name . ' đã bình luận về ' . $type . ' của bạn.';
                ...
                switch ($type) {
                    case 'post':
                        ...
                        $message = Auth::user()->first_name . ' ' . Auth::user()->last_name . ' đã phản hồi về bình luận ' . $type . ' của bạn.';
                        $participants[] = Auth::id();
                        break;
                    case 'blog':
                        $model = Blog::find($id);
                ...
                if ($reply_to != Auth::user()->username) {
                    ...
                    if (count($participants) > 1) {
                        ...
                        $latestComment = User::find(array_shift($participants));
                        $remainingCommentsCount = count($participants);
                        $message = $latestComment->first_name . ' ' . $latestComment->last_name . ' và ' . $remainingCommentsCount . ' người khác đã phản hồi về bình luận ' . $type . ' của bạn.';
                        if (!is_null($notification)) {
                            $notification->update([
                                'content' => $message,
                                'updated_at' => Carbon::now('Asia/Ho_Chi_Minh'),
